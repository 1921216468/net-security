<template>
    <div class="table-content">
        ...
        >
            ...
            <template #file="{ row }">
                <!-- 在这里渲染视频，你可以使用 row.file -->
                <video :src="row.file" controls class="video-column"></video>
            </template>
            <template #status="{ row }">
                <lay-switch
...
export default {
    setup() {
        ...
        const columns = ref([
            ...
            { title: '视频', width: '260px', key: 'file', customSlot: 'file' },
        ...
        const fetch_article = async (pageSize) => {
            LessonApi.getLessonList({ current: page.current, pageSize: pageSize }).then((res) => {
        ...
        const remove = () => {
            ...
            LessonApi.deleteLessonById({id:selectedKeys.value[0]}).then((res) => {
                layer.msg('Success', { icon: 1 }, () => {
