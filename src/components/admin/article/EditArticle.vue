...
const fetch_article = async () => {
    try {
        const res = await ArticleApi.getArticleById(route.query.id);
        article.value = res.data;
        if (res.data.imgList === null) {
            imageList = [{ url: "" }];
        } else {
            imageList = res.data.imgList;
            const processedData = imageList.map((item) => ({ url: item }));
...
const save_edit_note = async () => {
    ArticleApi.updateArticleById({
        id: article.value.id,
...
const updateDatabaseWithNewImageList = () => {
    ...
    // 调用更新数据库中图片 URL 字符串的 API
    ArticleApi.updateArticleById({
        id: article.value.id,
        title: article.value.title,
        content: article.value.content,
        img: updatedImageListString,
        tags:[]
    }).then((res) => {
        fetch_article();
        // 清空 imageList，然后添加一个空的上传框
        imageList = [{ url: '' }];
    });
};
