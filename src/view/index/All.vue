<template>
  <!-- 筛选框 -->
    ...
    <lay-container :fluid="true" style="padding-left: 200px;padding-right: 200px; padding-top: 10px">
        ...
        <lay-row :space="10">
            <lay-row :space="10">
                <lay-col :md="4" v-for="(item, index) in lesson_dataSource" :key="index">
                    <lay-card class="card-list-item" @click="goToLessonDetail(item.id)">
                        <img
                                :src="item.img"
                        />
...
export default {
    setup() {
        ...
        const fetch_lesson = async (input) => {
            LessonApi.getLessonList({searchText: input,current: currentPage.value, pageSize: pageSize.value }).then((res) => {
